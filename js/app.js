(() => {
    ...
    da.init();
    (function() {
        const partnersShowMore = document.querySelector(".partners__showmore");
        let partnersItems = 2;
        partnersShowMore.addEventListener("click", (() => {
            partnersItems += 2;
            const array = Array.from(document.querySelectorAll(".partners__list-item"));
            const visItems = array.slice(0, partnersItems);
            visItems.forEach((el => el.style.display = "block"));
            if (visItems.length === array.length) partnersShowMore.style.display = "none";
        }));
        const teamShowMore = document.querySelector(".team__showmore-btn");
        let teamItems = 1;
        teamShowMore.addEventListener("click", (() => {
            teamItems += 1;
            const array = Array.from(document.querySelectorAll(".team__grid-item"));
            const visItems = array.slice(0, teamItems);
            visItems.forEach((el => el.style.display = "flex"));
            if (visItems.length === array.length) teamShowMore.style.display = "none";
        }));
    })();
    window["FLS"] = true;
    isWebp();
